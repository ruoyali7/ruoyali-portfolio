...
const workflow = [
  ...
  ["05", "Distribute", "Approved assets move through a coordinated workflow across four social platforms."],
  ["06", "Learn", "Publishing, audience, attribution, and affiliate signals inform the next iteration."],
];
...
const contributions = [
  ...
  ["0→1 product iteration", "Built around problems I encountered while operating the workflow myself: fragmented assets, repeated manual steps, disconnected states, and weak feedback between creative and performance."],
];
...
export default function Home() {
  return <>
    ...
    <main>
      <section id="drama" className="case-intro"><div><p className="eyebrow">Featured case study / 01</p><h2>DramaClips<br/><em>DramoraAI</em></h2></div><div><p className="case-kicker">AI-assisted creative operations platform for short-form video distribution.</p><p>I built DramaClips as a 0→1 product to run my own short-form affiliate content workflow — from source media and AI-assisted hook creation through multi-platform publishing, attribution, and iteration.</p><p>Instead of treating AI as an isolated generator, I designed one connected system that keeps creative judgment in control throughout the workflow.</p><div className="case-actions"><a className="live-product-cta" href="https://dramaclips.vercel.app/" target="_blank" rel="noreferrer"><small>LIVE PRODUCT</small><span>Visit DramaClips ↗</span></a><a className="walkthrough-link" href="#interactive-demo">Try the interactive walkthrough ↓</a></div><div className="case-meta"><span>ROLE<br/><b>Creator · Product Engineer</b></span><span>STACK<br/><b>Next.js · TypeScript · Python · Supabase · R2 · Railway</b></span></div></div></section>

      <section className="problem-section"><p className="eyebrow">The problem</p><h2>Creative work was fragmented across files, prompts, browser tabs, storage, and publishing tools.</h2><p>Running the workflow manually meant moving repeatedly between source media, editing tools, storage, publishing surfaces, tracking links, and performance data. The challenge was not simply generating content with AI; it was connecting the entire creative and distribution loop.</p></section>

      <section className="workflow-section"><div className="section-heading"><p className="eyebrow">The system</p><h2>One connected workflow<br/><em>instead of isolated AI tools.</em></h2></div><div className="workflow-map">{workflow.map(([number, title, copy]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{copy}</p></article>)}</div><div className="closed-loop"><b>Create → Review → Publish → Measure → Learn → Create</b><p>Performance is not the end of the workflow — it becomes input for the next creative decision.</p></div></section>
      ...
      <section className="contribution-section"><div className="section-heading"><p className="eyebrow">What I built</p><h2>From ambiguous workflow<br/><em>to usable system.</em></h2></div><div className="contribution-grid">{contributions.map(([title, copy]) => <article className={title.startsWith("0→1") ? "product-iteration" : ""} key={title}><h3>{title}</h3><p>{copy}</p></article>)}</div><div className="system-strip"><small>CREATIVE + MEDIA SYSTEM</small><p>LLM APIs · Faster-Whisper · PySceneDetect · FFmpeg · Cloudflare R2 · Supabase · Vizard · Publishing integrations</p></div><div className="impact-strip"><div><small>BEFORE</small><p>Files + spreadsheets + prompts + browser tabs + manual publishing</p></div><span>→</span><div><small>AFTER</small><p>Shared asset library + AI generation + human review + publish workflow</p></div></div></section>

      <section className="validation-section"><div><p className="eyebrow">Real-world validation</p><h2>Built for a real workflow,<br/><em>not a portfolio prompt.</em></h2><p>DramaClips supports my real CPS/affiliate promotion workflow for short-form drama content. I use the system to prepare and distribute promotional creative across four social platforms, track downstream audience actions, and connect content performance back to affiliate outcomes.</p><p className="platform-label">TikTok · Instagram · YouTube · Facebook</p></div><div className="validation-points"><article><b>4</b><span>social platforms</span><p>Coordinated distribution workflow</p></article><article><b>Real</b><span>audience signals</span><p>Visits, code copies, redirects, and publishing outcomes</p></article><article><b>Earned</b><span>affiliate commission</span><p>Conversions validated the end-to-end loop</p></article></div><div className="feedback-loop"><small>FROM PERFORMANCE BACK TO CREATIVE</small><p>The system connects what was distributed with what audiences did next. Those signals inform the next hook, source selection, and publishing decision.</p></div></section>
